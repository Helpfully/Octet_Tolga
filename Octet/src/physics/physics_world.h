namespace octet {
  ...
  public:
    ...
    int add_hinge
    (
      const mat4t &modelToWorld, const vec4 &param, bool is_dynamic, body_kind kind, const vec3 &pivot, const vec3 &axis, btScalar optMass = 1.0f
    ) {
      ...
      btScalar mass = is_dynamic ? optMass : 0.0f;
      ...
      btHingeConstraint* hinge = new btHingeConstraint(*rigid_body,btVector3(pivot.x(),pivot.y(),pivot.z()),btVector3(axis.x(),axis.y(),axis.z()),true);

      //rigid_body->setAngularVelocity(btVector3(3,0,0));
      //rigid_body->applyImpulse(btVector3(10, 0, 0), pos);
    ...
    int add_rigid_body(const mat4t &modelToWorld, const vec4 &param, bool is_dynamic, body_kind kind, btScalar optMass = 1.0f) {
      btCollisionShape *shape = 0;
      switch (kind) {
        case body_box: shape = new btBoxShape(btVector3(param[0], param[1], param[2])); break;
        case body_sphere: shape = new btSphereShape(param[0]); break;
