...
#include "box_tumble/box_tumble_file_reader.h"

namespace octet {
  class box_tumble_box {
    material *mat;
    mesh *box_mesh;
    int physics_index;
  public:
  ...
  class box_tumble_app : public octet::app {
    ...
    // material for boxes
    material box_mat, hinge_mat, sphere_mat;
  
    // mesh for boxes
    mesh box_mesh, hinge_mesh, sphere_mesh;
    ...
    // storage for boxes
    dynarray<box_tumble_box> boxes;

    box_tumble_file_reader file_reader;

  public:

    // this is called when we construct the class
    box_tumble_app(int argc, char **argv) : app(argc, argv) {
    }

    bool read_file()
    {
      bool isSuccessful = false;
      isSuccessful = (file_reader.read_layout() == NULL) ? false : true;
      return isSuccessful;
    }
    ...
    void app_init() {
      ...
      bool fromFile = read_file();

      // make a mesh to share amoungst all boxes
      const float boxSize = 1.0f;

      box_mesh.make_cube(boxSize);
      box_mat.make_color(vec4(1, 0, 0, 1), false, false);

      const float hingeX = 10.0f, hingeY = 0.2f, hingeZ = 1.0f;

      hinge_mesh.make_aa_box(hingeX, hingeY, hingeZ);
      hinge_mat.make_color(vec4(0, 0, 1, 1), false, false);

      const float sphereRadius = 1.0f;

      sphere_mesh.make_sphere(sphereRadius);
      sphere_mat.make_color(vec4(0, 1, 0, 1), false, false);
      ...
      if(!fromFile)
      {
        printf("Emergency protocol!! Using default demo!\n");
        // make some boxes in the physics world with meshes for drawing.
        int num_boxes = 10;
        float box_spacing = 1.0f;
        boxes.resize(num_boxes+1); //+1 for the floor
        ...
        for (int i = 0; i != num_boxes; ++i) {
          int body = world.add_rigid_body
          (
            modelToWorld, 
            vec4(1.0f, 1.0f, 1.0f, 0), 
            true, 
            physics_world::body_box
          );
      ...
      {
        printf("Setting the scene according to the file!\n");
        // make some boxes in the physics world with meshes for drawing.
        int num_boxes = file_reader.box_number, box_counter = 0;
        float box_spacing = 2.0f;
        boxes.resize(num_boxes+1); //+1 for the floor

        mat4t modelToWorld;
        modelToWorld.loadIdentity();
        modelToWorld.translate(-0.5f * box_spacing * (BOX_TUMBLE_LAYOUT_SIZE), 0.0f, 0);

        game_layout *head = file_reader.get_head();
        
        int row_counter = 0;

        while(head != NULL && box_counter < num_boxes)
        {
          row_counter++;
          for(int i = 0; i < BOX_TUMBLE_LAYOUT_SIZE && box_counter < num_boxes; i++)
          {
            char value = head->row[i];
            if(value == '0')
            {
              int body = world.add_rigid_body(modelToWorld, vec4(sphereRadius, 0, 0, 0), true, physics_world::body_sphere, 0.1f);
              boxes[box_counter++].init(&sphere_mat, &sphere_mesh, body);
            }
            else if(value == '1')
            {
              int body = world.add_rigid_body(modelToWorld, vec4(boxSize, boxSize, boxSize, 0), true, physics_world::body_box, 0.001f);
              boxes[box_counter++].init(&box_mat, &box_mesh, body);
            }
            else if(value == '2')
            {
              int body = world.add_hinge(modelToWorld, vec4(hingeX, hingeY, hingeZ, 0), true, physics_world::body_box, vec3(hingeX, 0, 0), vec3(0, 0, 1), 5);
              boxes[box_counter++].init(&hinge_mat, &hinge_mesh, body);
            }
            else if(value == '3')
            {
              int body = world.add_hinge(modelToWorld, vec4(hingeX, hingeY, hingeZ, 0), true, physics_world::body_box, vec3(-hingeX, 0, 0), vec3(0, 0, 1), 5);
            ...
            {
              modelToWorld.rotateZ(90);
              int body = world.add_hinge
              (
                modelToWorld, vec4(hingeX, hingeY, hingeZ, 0), true, physics_world::body_box, vec3(hingeX, 0, 0), vec3(0, 0, 1), 5
              );
              boxes[box_counter++].init(&hinge_mat, &hinge_mesh, body);
              modelToWorld.rotateZ(-90);
            }
          ...
          modelToWorld.translate(box_spacing * BOX_TUMBLE_LAYOUT_SIZE * -1, box_spacing, 0);
          head = head->next;
        }

        /*for (int i = 0; i != num_boxes; ++i) {
          int body = world.add_rigid_body(modelToWorld, vec4(1.0f, 1.0f, 1.0f, 0), true, physics_world::body_box);
          boxes[i].init(&box_mat, &box_mesh, body);
          modelToWorld.translate(box_spacing, 0, 0);
        }*/
    ...
    void draw_world(int x, int y, int w, int h) {
      ...
      //boxes[10].get_modelToWorld(world, cameraToWorld);
      ...
      */
      ...
      //printf(rotatedMovement.toString());
